...
class FeReaderWindow(QMainWindow):
    ...
    def _build_continuous_pdf_widgets(self):
        ...
        if self.view_orientation == "horizontal":
            indices = range(0, len(self.pdf_images), 2)
        else:
            indices = range(len(self.pdf_images))

        for idx in indices:
            if self.view_orientation == "horizontal":
                pix = self._get_spread_pixmap(idx)
            else:
                img = self.pdf_images[idx]
    ...
    def _get_spread_pixmap(self, left_index):
        ...
        if not (0 <= left_index < len(self.pdf_images)):
            return None

        left_img = self.pdf_images[left_index]
        left_pix = QPixmap.fromImage(left_img)

        right_pix = None
        if left_index + 1 < len(self.pdf_images):
            right_img = self.pdf_images[left_index + 1]
            right_pix = QPixmap.fromImage(right_img)
            ...
            if self.view_mode == "single":
                ...
                if self.view_orientation == "horizontal":
                    ...
                    if 0 <= self.current_index < len(self.pdf_images):
                        img = self.pdf_images[self.current_index]
                        pix = QPixmap.fromImage(img)
                    else:
                        pix = None
    ...
    def set_view_orientation(self, mode: str):
        """Vertical = single page, Horizontal = two-page spread left-right."""
